---
title: Create New Quiz with Authoring Copilot
permalink: /create-new-quiz-with-authoring-copilot/
variant: markdown
description: ""
third_nav_title: Author
---
<h1>Create New Quiz with Authoring Copilot</h1>

<u>Important:</u>
* As Authoring Copilot uses generative AI, its response is probabilistic in nature. Thus, it may at times generate a different number of Sections, Activities and Components than expected.
* Check that the content generated by Authoring copilot is accurate and appropriate.
<ol><li>
In the Module Editor page of the Quiz, click the orange Pencil icon <img alt="Icon_Pencil" style="width:1.5rem; display: inline;" src="/images/2Teacher/Icon_Pencil.png"> on the Quiz card to open Authoring Copilot.
<img alt="Create New Quiz with Authoring Copilot" style="width: 100%;" src="/images/2Teacher/AU_ACP_Quiz1.png">
</li>
<li><div>
If you have used Authoring Copilot to create the Quiz, the Quiz Title and Quiz Notes would have been pre-populated. If you did not use Authoring Copilot to create the Section, you can add the Quiz Title and Quiz Notes.
	<br>
	<br>Quiz Notes will help guide the Authoring Copilot in the generation of the questions in the Quiz.<img alt="Create New Quiz with Authoring Copilot" style="width: 100%;" src="/images/2Teacher/AU_ACP_Quiz2.png">
</div></li>
<li>
Click the <b>Up/Down</b> arrows to set the Quiz Duration. This duration will be reflected at the Quiz Cover page. The recommended duration for each generated question within the Quiz may not add up to the total Quiz Duration set here.
<img alt="Create New Quiz with Authoring Copilot" style="width: 100%;" src="/images/2Teacher/AU_ACP_Quiz3.png">
</li>
<li>
In the Additional Details field, you can include information about the question types and number of questions in the Quiz.
	
<u>Note:</u>
* For example, “Create 3 Free Response Questions and 4 Multiple Choice Questions”. 
* Make sure that the information given about the number of questions in Additional Details corresponds to the Number of Components field.
* Authoring Copilot is unable to generate suggested answers for Free Response Questions.

</li>
<li>
You can add a knowledge base by clicking the checkbox next to Use Knowledge Base, followed by clicking <b>Add Knowledge Base</b>. 

<u>Note:</u>
* Adding a knowledge base will anchor the generation of content by Authoring Copilot to information which can be added by teachers, e.g. a syllabus document, a comprehension passage or an excerpt from MOE’s interactive digital textbooks. 
* Currently in R1, the knowledge base can only take in text input.  
* A maximum of 30,000 characters can be input into this field.
<img alt="Create New Quiz with Authoring Copilot" style="width: 100%;" src="/images/2Teacher/AU_ACP_Quiz5a.png">

Add the knowledge base into the field and click <b>Add</b>.
<img alt="Create New Quiz with Authoring Copilot" style="width: 100%;" src="/images/2Teacher/AU_ACP_Quiz5b.png">

Authoring Copilot will create a Suggested Quiz Plan with Quiz Description and Question(s). Click the checkbox next to Split into pages if you want the content to be populated across different pages of the Quiz. Click the checkboxes next to the generated content you wish to include in the Quiz. Click <b>Add</b> to autogenerate the Quiz Description and Question(s) selected.
<img alt="Create New Quiz with Authoring Copilot" style="width: 100%;" src="/images/2Teacher/AU_ACP_Quiz6.png">

You can edit the Quiz and Questions generated in the Module Editor page, as you would for a normal quiz.  

Alternatively, you can click <b>Back to Generate Quiz</b> to edit the Quiz Details and regenerate the Quiz. Newly generated questions will be appended to the end of the existing set of questions.
<img alt="Create New Quiz with Authoring Copilot" style="width: 100%;" src="/images/2Teacher/AU_ACP_Quiz7.png">
<hr></li></ol>