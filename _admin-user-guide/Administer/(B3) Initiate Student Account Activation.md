...
Onboarding via Login with MIMS

Note: From R4 (Jun 2026), Students with MIMS accounts can now access SLS directly using **Login with MIMS** without first completing SLS onboarding (e.g. setting up an SLS password or security questions). Students may choose to complete SLS onboarding at a later time if they wish to access SLS using **Login with SLS**.

Onboarding via Login with SLS

<p>In order to activate a Student's account using their SLS User ID, a One-Time Password (OTP) needs to be generated by either a Teacher or the SLS School Admin. ~~Students will be able to login with MIMS only after they have activated their SLS account using the OTP.~~</p>
<ol>
